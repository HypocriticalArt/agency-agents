#!/usr/bin/env bash
#
# Validates agent SKILL.md files (CC Switch / Agent Skills layout):
#   1. YAML frontmatter must exist with name, description (ERROR)
#   2. name must match parent directory name (ERROR)
#   3. metadata.color recommended for Agency agents (WARN if missing)
#   4. Recommended sections checked but only warned (WARN)
#   5. File must have meaningful content
#
...
lint_file() {
  ...
  # 2b. CC Switch / Agent Skills: name must match parent directory
  local dir_name fm_name
  dir_name="$(basename "$(dirname "$file")")"
  fm_name="$(echo "$frontmatter" | awk -F': ' '/^name: / {print $2; exit}' | tr -d "'\" ")"
  if [[ -n "$fm_name" && "$fm_name" != "$dir_name" ]]; then
    echo "ERROR $file: frontmatter name '${fm_name}' does not match directory '${dir_name}'"
    errors=$((errors + 1))
  fi

  if ! echo "$frontmatter" | grep -qE '^metadata:' || ! echo "$frontmatter" | grep -qE '^  color:'; then
    echo "WARN  $file: missing metadata.color (Agency convention)"
    warnings=$((warnings + 1))
  fi
